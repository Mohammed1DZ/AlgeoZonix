...
export default function ProtectedRiderLayout({ children }: { children: React.ReactNode; }) {
  ...
  const isDriverRole = userData?.role === 'driver';
  
  // Complete verification when all checks are done
  const isFullyVerified = isAuthVerified && isFirestoreReady && isUserDataLoaded && isDriverRole;
  ...
  // Redirect if auth check is complete but user is not an authenticated driver
  useEffect(() => {
        ...
        <Alert variant="destructive" className="max-w-md">
          ...
          <AlertDescription>
            {!isAuthVerified 
              ...
              : !isDriverRole 
              ? 'Insufficient permissions. Driver access required.'
              : 'Failed to verify driver status. Please try logging in again.'}
          </AlertDescription>
        </Alert>
      </div>
    );
  ...
  // Fully verified: render the protected rider dashboard
